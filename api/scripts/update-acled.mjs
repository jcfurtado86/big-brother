...
//
// Credenciais via env: ACLED_USER e ACLED_PASS (ou carrega do .env)
...
const ACLED_USER = process.env.ACLED_USER;
const ACLED_PASS = process.env.ACLED_PASS;

if (!ACLED_USER || !ACLED_PASS) {
  console.error('[ACLED] ACLED_USER e ACLED_PASS precisam estar no .env ou como variáveis de ambiente');
  process.exit(1);
}
...
async function main() {
  ...
  console.log(`\n[ACLED] Concluido: ${ok.length}/${REGIONS.length} regioes`);
  if (fail.length > 0) {
