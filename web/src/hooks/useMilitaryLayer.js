...
import { getMilitaryIcon, MILITARY_CATEGORY_COLOR } from '../providers/militaryIcons';
import { useBillboardLayer } from './useBillboardLayer';
import { SELECTED_PLANE_COLOR } from '../providers/constants';
import { getSetting } from '../providers/settingsStore';

export function useMilitaryLayer(viewer, pointsMap, visibleTypes) {
  const config = useMemo(() => ({
    batchSize: 50,
    labelBatchSize: 40,
    categoryColors: MILITARY_CATEGORY_COLOR,
    selectedColor: SELECTED_PLANE_COLOR,

    createBillboard(billboards, id, point, typesRef) {
      const category = point.category;
      const sz = getSetting('MILITARY_ICON_SIZE');
      ...
      const billboard = billboards.add({
        id,
        position: pos,
        image: getMilitaryIcon(),
        width: sz,
        height: sz,
        show,
        color: MILITARY_CATEGORY_COLOR[category] ?? MILITARY_CATEGORY_COLOR.base,
      });
      ...
      return {
        ...
        entry: {
          ...
          label: null,
          _h: sz,
          _name: point.name || point.ref || category,
